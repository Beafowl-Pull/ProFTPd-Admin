...
if (!$ac->is_valid_id($id)) {
    ...
    $keys = $ac->get_user_keys_by_userid($user[$field_userid]);
    if (!is_array($user)) {
...
<?php if (is_array($user)) { ?>
    ...
    <div class="col-xs-12 col-sm-6">
        ...
        <div class="panel panel-default">
            <div class="panel-heading">
                <h3 class="panel-title">
                    <a data-toggle="collapse" href="#userkeys" aria-expanded="true" aria-controls="userstats">SFTP user Public Keys</a>
                </h3>
            </div>
            <div class="panel-body collapse in" id="userkeys" aria-expanded="true">
                <div class="col-sm-12">
                    <div class="form-group">
                        <table class="table table-striped table-condensed sortable">
                            <thead>
                            <th>ID KEY</th>
                            <th><span class="glyphicon glyphicon-certificat" aria-hidden="true" title="User Key"></th>
                            ...
                            foreach ($keys as $key) { ?>
                                <tr>
                                    <td class="pull-middle"><?php echo $key['id']; ?></td>
                                    <td class="pull-middle hidden-xs hidden-sm"><?php echo substr($key['sftp_key'],0,60);?></td>
                                    <td class="pull-middle">
                                        <div class="btn-toolbar pull-right" role="toolbar">
                                            <a class="btn-group" role="group" href="edit_user_key.php?action=remove&user=<?php echo $user[$field_userid]; ?>&key=<?php echo $key['id']; ?>"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></a>
                                        </div>
                                    </td>
                                </tr>
                            <?php } ?>
                    ...
                    <div class="form-group">
                        <div class="col-sm-12">
                            <a class="btn btn-primary pull-right " href="edit_user_key.php?action=add&user=<?php echo $user[$field_userid]; ?>">Add KEY</a>
                        </div>
                    </div>
                </div>
            </div>
